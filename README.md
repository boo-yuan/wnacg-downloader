...
质量门禁覆盖整个 `wnacg` 包，包括入口和 Qt UI；分支覆盖率不得低于 55%。当前回归集为 102 项测试，最近一次全包分支覆盖率为 62.33%。
...
- 下载队列按“正在下载、等待任务、已完成任务”分层排列；等待任务保持 FIFO，新加入任务不会插队，状态变化时分页列表会自动重排。
...
- 搜索卡片实时区分已加入队列、已下载、本地文件缺失和下载失败；整页批量状态快照会在卡片销毁前取消，所有下载与 worker 信号强制回到 GUI 主线程，避免切换页面触发 Shiboken 原生崩溃。
- 重构为 `domain`、`application`、`infrastructure`、`ui` 分层，并从入口显式注入仓储与下载服务。
- 建立 Ruff、严格 Pyright、102 项 pytest 回归测试、全包分支覆盖率门槛、源码烟雾测试、依赖审计和 SBOM 验证；EXE 构建与烟雾测试由发布者在本地执行。
