...
质量门禁覆盖整个 `wnacg` 包，包括入口和 Qt UI；分支覆盖率不得低于 55%。当前回归集为 114 项测试，最近一次全包分支覆盖率为 64.69%。
...
- 空搜索结果标题统一使用当前主题主色；搜索漫画卡片缩至 188 px 宽，封面按比例显示在 164×209 px 区域内。
...
- 暂停续传改为在数据库原子记录每张图片的实际输出文件名，恢复时验证现有文件并只下载缺失项，不再因重新推算路径而从零开始。
- “保留原始名称”不再添加 `0001-` 类四位前缀；旧前缀文件续传时可识别并安全迁移，原名冲突时仅追加 `(2)` 后缀。
- `.wnacg-manifest.json` 承担防止误删用户文件的所有权保护，现已迁移到应用数据目录；新漫画文件夹不再创建该文件，旧文件会在启动后自动迁移。
- 直链列表会过滤站点混入的 `shoucang.jpg` 收藏按钮资源，图片数准确时不再误触发逐页回退请求。
- 漫画目录改为纯标题命名，同名任务使用 `(2)`、`(3)` 后缀安全避让；未完成目录使用 `_` 前缀，完成后安全改名，并可在启动时恢复中断于改名过程的任务。
- GitHub CI 专注代码质量、测试、依赖审计和 SBOM；Windows 单文件 EXE 改为使用锁文件在本地手动构建与验证。
- 回归集增至 114 项，Ruff、严格 Pyright、源码烟雾测试和 55% 全包分支覆盖率门槛均通过，最近一次覆盖率为 64.69%。
