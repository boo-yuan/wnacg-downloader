...
![Version](https://img.shields.io/badge/version-1.4.0-2563eb.svg)
...
| 文件所有权元数据 | 应用数据目录中的 `artifacts` 目录，不写入漫画文件夹 |
...
- 下载目录只使用漫画标题命名；未完成时在目录名前加 `_`，全部图片校验通过后再原子恢复正式名称；同名或已有目录自动追加 `(2)`、`(3)`，不会混入 `aid`，也不会互相覆盖。
- 文件所有权元数据只记录程序拥有的文件，保存在应用数据目录。打包、重下和取消清理不会处理用户自行放入任务目录的文件或无所有权记录的 ZIP。
...
质量门禁覆盖整个 `wnacg` 包，包括入口和 Qt UI；分支覆盖率不得低于 55%。当前回归集为 115 项测试，最近一次全包分支覆盖率为 64.75%。
...
<details open>
<summary><strong>v1.4.0 · 当前最新版</strong></summary>

本版本重点完善下载恢复与文件目录生命周期，解决暂停后重复下载、原始文件名被添加序号前缀等问题，并优化搜索卡片与 2K、4K 高 DPI 显示体验。

- 暂停续传会原子记录每张图片的实际输出文件名；恢复时校验已有文件，只下载缺失或损坏项，不再从零开始。
- “保留原始名称”不再添加 `0001-` 类四位前缀；旧版前缀文件仍可识别并安全迁移，重名时仅追加 `(2)` 等避让后缀。
- 未完成漫画目录统一使用 `_漫画名`，全部图片校验完成后再安全改为正式名称；启动时可自动恢复中断于目录改名过程的任务。
- `.wnacg-manifest.json` 所有权元数据迁移到应用数据目录，新下载目录不再创建该文件；旧清单会自动迁移，并继续保护用户自行放入的文件。
- 数据库升级并持久化图片输出名称，同时拒绝目录穿越类非法文件名，增强恢复和文件写入安全性。
- 空搜索结果标题改用当前主题主色；搜索卡片缩至 188 px，封面在 164×209 px 区域内保持比例平滑显示。
- 窗口采用 DPI 无关逻辑尺寸，默认 `1200×720`、最小 `960×600`；适配 2K、4K 和 Windows 高缩放，可用空间不足时自动缩小并保留边距。
- 回归集增至 115 项，Ruff、严格 Pyright、源码烟雾测试和 55% 全包分支覆盖率门槛均通过，最近一次覆盖率为 64.75%。
